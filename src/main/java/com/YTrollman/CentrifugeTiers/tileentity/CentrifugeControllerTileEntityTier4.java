...
public class CentrifugeControllerTileEntityTier4 extends CentrifugeControllerTileEntity {
    ...
    private final IntArray times = new IntArray(7) {
        ...
        public int get(int index) {
            if(getCount() - index >= 1) return CentrifugeControllerTileEntityTier4.this.time[index];
            return 0;
        }

        @Override
        public void set(int index, int value) {
            if(getCount() - index >= 1) CentrifugeControllerTileEntityTier4.this.time[index] = value;
        }
    ...
    public void tick() {
        if (level != null && !level.isClientSide()) {
            ...
            validateTime++;
            if (validateTime >= 10) validateStructure(this.level);
            if (dirty) {
                this.dirty = false;
                this.setChanged();
            }
