...
public class CentrifugeControllerTileEntityTier3 extends CentrifugeControllerTileEntity {
    ...
    private final IntArray times = new IntArray(6) {
        ...
        public int get(int index) {
            if(getCount() - index >= 1) return CentrifugeControllerTileEntityTier3.this.time[index];
            return 0;
        }

        @Override
        public void set(int index, int value) {
            if(getCount() - index >= 1) CentrifugeControllerTileEntityTier3.this.time[index] = value;
        }
    ...
    public void tick() {
        if (level != null && !level.isClientSide()) {
            ...
            validateTime++;
            if (validateTime >= 20) validateStructure(this.level);
            if (dirty) {
                this.dirty = false;
                this.setChanged();
            }
            ...
            if (level != null) {
                ...
                for (int j = 0; j < recipe.itemOutputs.size(); j++) {
                    ...
                    if (chance >= level.random.nextFloat()) {
                        ...
                        if (j == 2 && !recipe.noBottleInput) glassBottle.shrink(recipes.get(i).itemOutputs.get(2).getLeft().getCount() * multiplier);
                    }
                }
                ...
                for (Pair<FluidStack, Float> fluidStackFloatPair : recipe.fluidOutput) {
                    ...
                    if (chance >= this.level.random.nextFloat()) {
                        ...
                        if (tank != -1) this.fluidTanks.fill(tank, fluid, IFluidHandler.FluidAction.EXECUTE);
                    }
                }
    ...
    protected void processRecipe(int i) {
        if (canProcess(i)) {
            energyStorage.consumeEnergy(Config.RF_TICK_CENTRIFUGE.get() * CentrifugeConfig.CENTRIFUGE_TIER_3_RF_PER_BLOCK.get());
            ++time[i];
