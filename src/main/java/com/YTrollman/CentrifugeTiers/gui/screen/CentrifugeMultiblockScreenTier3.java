...
public class CentrifugeMultiblockScreenTier3 extends ContainerScreen<CentrifugeMultiblockContainerTier3> {
    public CentrifugeMultiblockScreenTier3(CentrifugeMultiblockContainerTier3 screenContainer, PlayerInventory inventory, ITextComponent titleIn) {
    ...
    protected void drawGuiContainerBackgroundLayer(@Nonnull MatrixStack matrix, float partialTicks, int mouseX, int mouseY) {
        ...
        if (client != null) {
            ...
            int scaledprogress1 = 16 * this.container.getTime(0) / Math.max(this.container.centrifugeTileEntityTier3.totalTime,1);
            this.blit(matrix, i + 52, j + 26, 176, 1, 16, scaledprogress1);
            int scaledprogress2 = 16 * this.container.getTime(1) / Math.max(this.container.centrifugeTileEntityTier3.totalTime,1);
            this.blit(matrix, i + 88, j + 26, 176, 1, 16, scaledprogress2);
            int scaledprogress3 = 16 * this.container.getTime(2) / Math.max(this.container.centrifugeTileEntityTier3.totalTime,1);
