...
public class CentrifugeMultiblockScreenTier5 extends ContainerScreen<CentrifugeMultiblockContainerTier5> {
    ...
    protected void drawGuiContainerBackgroundLayer(@Nonnull MatrixStack matrix, float partialTicks, int mouseX, int mouseY) {
        ...
        if (client != null) {
            client.getTextureManager().bindTexture(new ResourceLocation(CentrifugeTiers.MOD_ID,"textures/gui/multiblock_centrifuge_tier_5.png"));
            int i = this.guiLeft;
            int j = this.guiTop;
            this.blit(matrix, i, j, 0, 0, this.xSize, this.ySize);
            int scaledprogress1 = 16 * this.container.getTime(0) / Math.max(this.container.centrifugeTileEntityTier5.totalTime,1);
            this.blit(matrix, i + 44, j + 26, 176, 1, 16, scaledprogress1);
            int scaledprogress2 = 16 * this.container.getTime(1) / Math.max(this.container.centrifugeTileEntityTier5.totalTime,1);
            this.blit(matrix, i + 62, j + 26, 176, 1, 16, scaledprogress2);
            int scaledprogress3 = 16 * this.container.getTime(2) / Math.max(this.container.centrifugeTileEntityTier5.totalTime,1);
            this.blit(matrix, i + 80, j + 26, 176, 1, 16, scaledprogress3);
            int scaledprogress4 = 16 * this.container.getTime(3) / Math.max(this.container.centrifugeTileEntityTier5.totalTime,1);
            this.blit(matrix, i + 98, j + 26, 176, 1, 16, scaledprogress4);
            int scaledprogress5 = 16 * this.container.getTime(4) / Math.max(this.container.centrifugeTileEntityTier5.totalTime,1);
            this.blit(matrix, i + 116, j + 26, 176, 1, 16, scaledprogress5);
            int scaledprogress6 = 16 * this.container.getTime(5) / Math.max(this.container.centrifugeTileEntityTier5.totalTime,1);
            this.blit(matrix, i + 134, j + 26, 176, 1, 16, scaledprogress6);
            int scaledprogress7 = 16 * this.container.getTime(6) / Math.max(this.container.centrifugeTileEntityTier5.totalTime,1);
            ...
            this.blit(matrix, i + 10, j + 41 + (58-scaledRF), 176, 28 + (58-scaledRF), 12, scaledRF);
        }
    }
    ...
    public void render(@Nonnull MatrixStack matrix, int mouseX, int mouseY, float partialTicks) {
        ...
        if (mouseX >= this.guiLeft + 10 && mouseX <= this.guiLeft + 22 && mouseY >= this.guiTop + 41 && mouseY <= this.guiTop + 98){
