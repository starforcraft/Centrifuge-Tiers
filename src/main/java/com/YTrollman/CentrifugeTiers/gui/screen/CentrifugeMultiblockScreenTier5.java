...
public class CentrifugeMultiblockScreenTier5 extends ContainerScreen<CentrifugeMultiblockContainerTier5> {
    ...
    protected void init() {
        super.init();
        int buttonX = this.guiLeft + xSize - TAB_BG_WIDTH - 6;
    ...
    private void drawInputSlots(MatrixStack matrix, int left, int top) {
        this.drawTexture(matrix, left + L_BDR_WD + SLOT_WD - 17, top + TOP_PAD, 62, 0, 18, 18); //draw bottle slot
        for (int i = 0; i < numInputs; i++) {
    ...
    private void drawPowerBar(MatrixStack matrix, int left, int top) {
        this.drawTexture(matrix, left + 9, top + 37, L_BDR_WD + (BG_FILL_WD - 1) + R_BDR_WD, 0, 14, 60);
