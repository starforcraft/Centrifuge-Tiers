...
public class CentrifugeControllerBlockTierCreative extends Block {
    ...
    public ActionResultType onBlockActivated(@Nonnull BlockState state, World world, @Nonnull BlockPos pos, @Nonnull PlayerEntity player, @Nonnull Hand hand, @Nonnull BlockRayTraceResult blockRayTraceResult) {
        if (!world.isRemote) {
            ...
            if (blockEntity != null) {
                if (blockEntity instanceof CentrifugeControllerTileEntityTierCreative){
                	CentrifugeControllerTileEntityTierCreative multiblockCentrifuge =  (CentrifugeControllerTileEntityTierCreative)blockEntity;
    ...
    public boolean removedByPlayer(BlockState state, World world, BlockPos pos, PlayerEntity player, boolean willHarvest, FluidState fluid) {
        if (!world.isRemote){
        	CentrifugeControllerTileEntityTierCreative controller = (CentrifugeControllerTileEntityTierCreative)world.getTileEntity(pos);
            if (controller !=null){
    ...
    public void onReplaced(@Nonnull BlockState state1, World world, @Nonnull BlockPos pos, @Nonnull BlockState state, boolean isMoving) {
        ...
        if (blockEntity instanceof CentrifugeControllerTileEntityTierCreative && state.getBlock() != state1.getBlock()){
        	CentrifugeControllerTileEntityTierCreative centrifugeTileEntity = (CentrifugeControllerTileEntityTierCreative)blockEntity;
    ...
    public TileEntity createTileEntity(BlockState state, IBlockReader world) {
        return new CentrifugeControllerTileEntityTierCreative();
    }
    ...
    public void addInformation(@Nonnull ItemStack stack, @Nullable IBlockReader worldIn, @Nonnull List<ITextComponent> tooltip, @Nonnull ITooltipFlag flagIn) {
        ...
        if (Screen.hasControlDown()){
            tooltip.addAll(new TooltipBuilder()
                    ...
                    .addTip(I18n.format("block.ctiers.centrifuge_tier_creative.tooltip.faster"), TextFormatting.AQUA)
                    .addTip(I18n.format("block.ctiers.centrifuge_tier_creative.tooltip.multi"), TextFormatting.AQUA)
                    .addTip(I18n.format("block.ctiers.centrifuge_tier_creative.tooltip.ItemMaxStackSize"), TextFormatting.AQUA)
